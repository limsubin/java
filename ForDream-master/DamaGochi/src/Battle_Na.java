...
public class Battle_Na {
	void Na() {
		Scanner scan = new Scanner(System.in);

		Dama_Battle battle = new Dama_Battle();
		int rd = Dama_Battle.randControl.nextInt(3);                                     // 랜덤변수
		...
		switch (Dama_Battle.menuControl) {                                                // switch문 이용
		case 1:                                                                           // 1. 애교 메뉴
			if (rd == 0) {
				...
				Dama_Battle.myHealth = Dama_Battle.myHealth - 5;                         // 체력이 -5 줄었다
				break;
			} else if (rd == 1) {
				...
				Dama_Battle.myHealth = Dama_Battle.myHealth - 3;                         //체력이 -3 줄었다
				break;
			} else if (rd == 2) {
				System.out.println("なちゃん : （なきながら）ふとったハトをみるのがすごいすきなんです");
				System.out.println(Dama_Status.name + "은(는) 심장이 없어졌다");
				System.out.println(Dama_Status.name + "체력 -8");                          //체력이 -8 줄었다
		...
		case 3:                                                                           // 3. 애교 메뉴
			if (rd == 0) {
				...
				Dama_Battle.health = Dama_Battle.health - 5;                              // 체력이 -5 줄었다
				break;
			} else if (rd == 1) {
				System.out.println(Dama_Status.name + " : 나쨩~!");
				System.out.println("なちゃん : えぇ？");
				...
				System.out.println(Dama_Status.name + "은(는) 심장이 없어졌다");
				System.out.println(Dama_Status.name + "체력 -3");
				Dama_Battle.myHealth = Dama_Battle.myHealth - 8;                         // 체력이 -8 줄었다
				break;
			}
		case 4:                                                                          // 4. 나가기 메뉴
			if (rd == 0) {
				System.out.println("なちゃん : いかないほうがいいよ！");
				System.out.println(Dama_Status.name + " は。。はい");
				System.out.println(Dama_Status.name + " 은(는) 도망치는데 실패했다");
				System.out.println(Dama_Status.name + "체력 -3");
				Dama_Battle.myHealth = Dama_Battle.myHealth - 3;                        // 체력이 -3 줄었다
				break;
			} else {
				System.out.println("なちゃん은(는) 명종와(과) 결혼에 성공했다");
